...
write.map<-function(map.list,file.out){
  ...
  if(!any(class(map.list)=="list" | class(map.list)=="sequence"))
    stop(deparse(substitute(map.list))," is not an object of class 'list' or 'sequnece'")
  if(!any(class(file.out)=="character"))
    stop(deparse(substitute(file.out))," is an invalid output file name")
  ...
  for(i in 1:length(map.list)){
    if(!any(class(map.list[[i]])=="sequence"))
      stop("Object ", i , " in map.list is not an object of class 'sequnece'")
    if(is.null(map.list[[i]]$seq.like))
      stop("Parameters are not estimated for object ", i, " in map.list")
